...
public interface SponsorAdsDao extends CrudRepository<SponsorAds, Integer> {
	@Query("select a from SponsorAds a where a.business.id=?1 and a.title like %?2% and (9=?3 or a.status=?3)")
	List<SponsorAds> queryAds(Pageable pageable, int businessId, String title,
			int status);
	@Query("select count(a) from SponsorAds a where a.business.id=?1 and a.title like %?2% and (9=?3 or a.status=?3)")
